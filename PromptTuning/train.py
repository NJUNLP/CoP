...
def main_traning(args):
    task_name = args.task_name
    log_path = args.log_path
    writer = SummaryWriter(log_path + task_name)
    log_w = open(log_path + "{}.txt".format(task_name),'w')
    ...
    def load(target):
        ...
        f1 = open(base_path + "{}/{}.source".format(target,target),'r',encoding='utf-8')
        document = f1.readlines()

        f2 = open(base_path + "{}/{}.target".format(target,target),'r',encoding='utf-8')
        summary = f2.readlines()

        f3 = open(base_path + "{}/{}.label".format(target,target),'r',encoding='utf-8')
        label = f3.readlines()

        f4 = open(base_path + "{}/{}.ref".format(target,target),'r',encoding='utf-8')
    ...
    high_bart_scorer = BARTScorer(checkpoint=args.model_path,PromptBART = True,Dev = True)
    ...
    def test(log_step):
        ...
        expect_predict_num = int(sum(total_label) * args.predict_ratio) + sum(total_label)
    ...
    if args.training_mode == 'few-shot':
                ...
                for i in train_index:
                    ...
                    if update_step% 100 == 0:
                        ...
                        if dev_loss <  best_loss:
                            ...
                            if free_space < 15:
                                log_w.write("!!!!!!!!!HIT ERROR WITH NO SPACE !!!!!!!!! With Loss {}  \n".format(best_loss))
                                print("!!!!!!!!!HIT ERROR WITH NO SPACE !!!!!!!!! With Loss {}".format(best_loss))
                            else:
